...
class JapantravelEventSource extends DefaultEventSource {
    ...
    async scrapEvent(): Promise<Array<EventType>> {
      ...
      for (let i = 1; i<5; i++) {
        ...
        const res = await fetch(`${myConfig.endpoint}&p=${i}/`);
        ...
        $("article").each((index, element) => {
          ...
          let val = $(element).find('.article-list__title').find('a').attr('href') || "";
          ...
          val = $(element).find('.article-list__title').find('a').text() || "";
          anEvent.description = val.trim();
